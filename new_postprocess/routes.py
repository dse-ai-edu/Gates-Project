...
from app.prompts import JUDGE, CONTENT_REJECTION_PROMPT
...
def comment_generate(system_info, answer_text, question_text, reference_text, history_prompt_dict, predefined_flag = "", suggestion = None):
    try:
        ...
        if grading_result["pass"] == False:
            return {
                "success": True, 
                'feedback_text': "We can not process this solution, please check the question and involve human experts in grading work.",
                'feedback_prob': None,
...
def api_image_convert():
    ...
    try:
        ...
        if allow_reject and input_type == 'answer':
            system_prompt += CONTENT_REJECTION_PROMPT
