  ...
  document.addEventListener("mouseout", (e) => {
    ...
    if (!activeSpan.contains(e.relatedTarget)) {
      if (activeSpan.dataset.savedTitle) {
        ...
        delete activeSpan.dataset.savedTitle;
      }
      hide();
    }
  });
...
async function displayConfigurationAsText() {
  ...
  templateDisplay.textContent = templates.length
  ...
  customDisplay.textContent = pattern_custom_flag
...
function generatePersonalizedFeedback() {
  ...
  const answerText = sessionStorage.getItem(INPUT_CACHE_KEYS.answer)?.trim();
