...
# default model
DEFAULT_MODEL = os.environ.get("DEFAULT_MODEL", "gemini-3.1-flash-lite-preview")
...
def comment_generate(system_info, answer_text, question_text, reference_text, history_prompt_dict, predefined_flag = "", suggestion = None):
    try:
        ...
        if "adaptive" in feedback_pattern.lower():
            selected_pattern_key = utils.decide_adaptive_pattern(question=question_text, answer=answer_text, model=DEFAULT_MODEL)
            from_adaptive = True
        else:
