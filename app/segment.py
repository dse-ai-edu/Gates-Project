from app.data_structure import VerticalSplitOutput
from app.llm_utils import llm_generate

import json

from app.image_prompts import SEGMENTATION
...
def get_vertical_splits_from_llm(
    ...
    result = llm_generate(
        system_prompt=SEGMENTATION,
        user_prompt="",
        image=image_path,
        model=model,
        text_format=VerticalSplitOutput,
    )

    if result.obj is None:
        raise ValueError(
            f"Structured output missing: {result}"
        )

    if isinstance(result.obj, BaseModel):
        raw_obj = result.obj.model_dump()

    elif isinstance(result.obj, dict):
        raw_obj = result.obj

    else:
        raise ValueError(
            f"Unexpected structured output type: {type(result.obj)}"
        )

    if "splits" not in raw_obj:
        raise ValueError(
            f"'splits' field missing: {raw_obj}"
        )
...
def process_pdf_dir(args):
    ...
    if not pdf_root.exists():
        raise FileNotFoundError(
            f"PDF directory not found: {pdf_root}"
        )
    ...
    for pdf_path in pdf_files:
        ...
        try:
            ...
            if splits_jsonl.exists():
                splits_jsonl.replace(
                    pdf_crop_dir / "splits.jsonl"
                )
